...
public class DoviRenderPipeline : RenderPipeline
{
    /// <summary>
    /// 公有成员
    /// </summary>
    ...
    /// <summary>
    /// DoviRenderPipeline构造函数
    /// </summary>
    /// <param name="asset"></param>
    #region DoviRenderPipeline
    public DoviRenderPipeline(DoviRenderPipelineAsset asset)
    {
        _renderer = new GIRenderer(this);
        _command = new CommandBuffer() { name = "DoviGI.RenderPipeline" };
        _filteringSettings = FilteringSettings.defaultValue;

        PerObjectData = asset.perObjectData;
        Shader.globalRenderPipeline = "DoviGI";
    ...
    protected override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
    {
        ...
        foreach (var camera in cameras)
        {
            ...
            {
                //开启GI
                if(camera.TryGetComponent<DoviGI>(out var gi) && gi.isActiveAndEnabled)
                {
            ...
            else
            {
                ...
                if(mainCamera != null && mainCamera.TryGetComponent<DoviGI>(out var gi) && gi.isActiveAndEnabled)
                {
    ...
    {
        ClearDirectionLight();
        if (LitcommandBuffer == null) LitcommandBuffer = new CommandBuffer() { name = "Dovi Render Pipeline Test" };                          //渲染开始后，创建CommandBuffer;
            ...
            if (light.lightType != LightType.Directional)                                                                           //判断灯光类型
            {
                if (light.lightType != LightType.Point)
                {
                    if (light.lightType == LightType.Spot)                                                                           //聚光灯
                    {
                        ...
                        SLightColors[sLightIndex].w = light.range;    
                        //将聚光灯的距离设置塞到颜色的A通道
        ...
        //渲染设置
        SortingSettings sortingSettings = new SortingSettings(camera) { criteria = SortingCriteria.CommonOpaque };                   //渲染时，会牵扯到渲染排序，所以先要进行一个相机的排序设置，这里Unity内置了一些默认的排序可以调用，CommonOpaque表示不透明物体的排序
        DrawingSettings drawingSettings = new DrawingSettings(new ShaderTagId("DoviLit"), sortingSettings);                   //这边进行渲染的相关设置，需要指定渲染的shader的光照模式
        //目前只绘制不透明物体                                                                                                                      //过滤                                                                                                                       
    ...
    private void ClearDirectionLight()
    {
        ...
        {
            PLightColors[i] = new Vector4(0,0,0,0);                                              //矩阵第4列为位置
            PLightPos[i] = new Vector4(0,0,0,0);
        }
        for (int i = 0; i < maxDirectionalLights; i++)
        {
            SLightColors[i] = new Vector4(0,0,0,0);                                              //矩阵第三列为朝向，第四列为位置
            SLightDirections[i] = new Vector4(0,0,0,0);                                                                    //内角弧度cos值
            SLightPos[i] = new Vector4(0, 0, 0, 0);
        }
